...
describe('CallbackView', () => {
  ...
  beforeEach(() => {
    ...
    Object.defineProperty(window, 'location', {
      value: {
        href: 'http://localhost:8050/callback?code=mock-code&state=mock-state'
      },
      writable: true
    });
  ...
  it('should handle login callback and redirect to home page', async () => {
    ...
    expect(oidcService.handleOidcCallback).toHaveBeenCalledWith(
      'http://localhost:8050/callback?code=mock-code&state=mock-state'
    );
  ...
  it('should handle logout callback and redirect to home page', async () => {
    ...
    Object.defineProperty(window, 'location', {
      value: {
        href: 'http://localhost:8050/logout-callback'
      },
      writable: true
    });
    ...
    // Assert
    expect(oidcService.handleOidcCallback).toHaveBeenCalledWith(
      'http://localhost:8050/logout-callback'
    );
    expect(router.currentRoute.value.path).toBe('/');
  });
