...
class OidcService {
  ...
  public async completeOidcCallback(url: string): Promise<void> {
    ...
    } else if (false) { // TODO complete logout condition
  ...
  public async login(): Promise<void> {
    ...
    try {
      ...
      const TODO_FIXME = "TODO placeholder to actual values"; // TODO placeholder to actual values

      const authorizationUrl = new URL(authorizationServer.authorization_endpoint!);
      authorizationUrl.searchParams.set('client_id', TODO_FIXME); // TODO placeholder to actual values
      authorizationUrl.searchParams.set('code_challenge', await oauth.calculatePKCECodeChallenge(pkceVerifyState.codeVerifier));
      authorizationUrl.searchParams.set('code_challenge_method', TODO_FIXME); // TODO placeholder to actual value
      authorizationUrl.searchParams.set('redirect_uri', TODO_FIXME); // TODO placeholder to actual value
      ...
      authorizationUrl.searchParams.set('state', TODO_FIXME); // TODO placeholder to actual value

      // TODO go to the keycloak's authorization endpoint to let the user login, so we get an auth code and state back
  ...
  public async loadSession(): Promise<SessionInfo | undefined> {
    ...
    if (!existingSession) {
      ...
      // TODO implement condition: session exists and access token is not yet expired
      //  don't forget to calculate in some tokenExpiryTimeSkewInSeconds,
      //  so the token does not expire while being transmitted to the backend
      true
    ) {
