  ...
  <div class="about">
    ...
    <div v-if="isLoggedIn && hasViewPermission">
      ...
      <div v-else class="permission-warning">
        <p>You don't have permission to add new text messages.</p>
      </div>
    ...
    <div v-else-if="isLoggedIn && !hasViewPermission" class="permission-error">
      <p>You don't have permission to view text messages.</p>
    </div>

    <div v-else class="login-prompt">
      <p>Please log in to view and create text messages.</p>
    </div>
  </div>
</template>
...
import {
  ...
  type TextMessage,
  textMessageService
} from '../services/textmessage.service';
...
const hasViewPermission = computed(() => permissionCheckService.hasViewTextMessagesPermission());
const hasEditPermission = computed(() => permissionCheckService.hasEditTextMessagesPermission());
...
const fetchMessages = async () => {
  ...
  try {
    const response: PaginatedResponse<TextMessage> = await textMessageService.getAllTextMessages(
      currentPage.value,
      pageSize
    );
...
const createMessage = async () => {
  ...
  try {
    await textMessageService.createTextMessage(newMessage.value);
    newMessage.value = '';
    await fetchMessages();
  } catch (err) {
